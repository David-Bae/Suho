...
def analysisGPT(questions, user_answers):
    #! Language Model
    ...
    prompt = PromptTemplate(
        template="""
다음은 고령자의 건강 상태 분석을 위한 상담 내역입니다. Q는 상담자가 질문한 내용이고, A는 고령자가 답변한 내용입니다.
======================================================
{QA}
======================================================

위 답변을 토대로 고령자의 건강 상태를 분석해서 고령자의 보호자에게 보여줄 보고서를 한 문단(3,4 문장으로 구성)으로 작성하세요.
""",
        input_variables=["QA"]
    )
    ...
    result = chat(
        [
            ...
            HumanMessage(content=prompt.format(QA=qa))
        ]
    )
